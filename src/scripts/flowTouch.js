...
window.addEventListener("pointerdown", (e) => {
    ...
    if (window["Disco"]) if (el.hasAttribute("haptic") || window.getComputedStyle(el).getPropertyValue("--flow-haptic") == "true") Disco.triggerHapticFeedback("KEYBOARD_PRESS")
...
const metroToggleSwitch = {
    ...
    animate: (el, from, to) => {
        ...
        const duration = 200 * DiscoBoard.backendMethods.animationDurationScale.get()
...
const metroDropdownMenu = {
    click: (el) => {
        ...
        setTimeout(() => {
            try { el.closest("div.flow-scrollable").DiscoScroll.refresh() } catch (error) { }
        }, 300 * DiscoBoard.backendMethods.animationDurationScale.get());
        ...
        setTimeout(() => {
            if (window["Disco"]) Disco.triggerHapticFeedback("CONFIRM")
        }, 200 * DiscoBoard.backendMethods.animationDurationScale.get());
    },
    blur: (el) => {
    ...
    select: (el, index, event = true, animate = true) => {
        ...
        if (animate) {
            ...
            if (window["Disco"]) if (event) setTimeout(() => { Disco.triggerHapticFeedback("CONFIRM") }, 200);
        } else {
        ...
        setTimeout(() => {
            el.querySelector("div.metro-dropdown-option").style.removeProperty("transition")
        }, 10 * DiscoBoard.backendMethods.animationDurationScale.get());

        if (event) setTimeout(() => { el.dispatchEvent(new CustomEvent("selected", { detail: { index: index, prevIndex: lastIndex } })) }, 200 * DiscoBoard.backendMethods.animationDurationScale.get());
    }
}
...
document.querySelectorAll("input[type=range]").forEach(el => {
    ...
    if (el.supportsFlowRangeInput) {
        el.addEventListener("input", () => {
            updateFlowRangeInputBackground(el)
            if (window["Disco"]) if (window["pointerDownRanges"]) if (window.pointerDownRanges.includes(el)) Disco.triggerHapticFeedback("CLOCK_TICK")
        })
        ...
        el.addEventListener("change", () => {
            updateFlowRangeInputBackground(el)
            //if (window["pointerDownRanges"]) if (window.pointerDownRanges.contains(el)) Disco.triggerHapticFeedback("CLOCK_TICK")
        })
        updateFlowRangeInputBackground(el)
    }
})
