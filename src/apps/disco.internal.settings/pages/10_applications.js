        ...
        parent.DiscoBoard.alert(
            ...
            [{
                title: window.i18n.t("common.actions.yes"), style: "default", inline: true, action: () => {
                    Disco.uninstallApp(packageName, parent.DiscoBoard.backendMethods.packageManagerProvider.get());
                    pageNavigation.settingsHome()
                }
...
function refreshAppList(params) {
    document.querySelector("#apps-tab > div.scroller > div.disco-list-view").innerHTML = "";
    var allappsarchive = !!window.parent.allappsarchive ? JSON.parse(JSON.stringify(window.parent.allappsarchive)) : window.parent.DiscoBoard.backendMethods.reloadAppDatabase()
    allappsarchive.map(e => {
        const appPreference = parent.DiscoBoard.backendMethods.getAppPreferences(e.packageName)
        if (appPreference.label != "auto") e.label = appPreference.label
        return e
    }).forEach(e => {
        const appPreference = parent.DiscoBoard.backendMethods.getAppPreferences(e.packageName)
        const appDetails = parent.DiscoBoard.backendMethods.getAppDetails(e.packageName)

        const el = DiscoElements.wListViewItem(appDetails.label, appPreference.label == "auto" ? "" : parent.DiscoBoard.backendMethods.getAppDetails(e.packageName, true).label)
        el.setAttribute("packagename", e.packageName)
        document.querySelector("#apps-tab > div.scroller > div.disco-list-view").append(el)
    });
    $("#apps-tab > div.scroller > div.disco-list-view > div.disco-list-view-item").on("flowClick", e => {
        try {
            document.querySelector("div.app-preference-form").style.setProperty("display", "none")
            delete window.lastSelectedApp

            const appdetail = parent.DiscoBoard.backendMethods.getAppDetails(e.target.getAttribute("packagename"))
            const appPreference = parent.DiscoBoard.backendMethods.getAppPreferences(e.target.getAttribute("packagename"))
            ...
            appNameChangerInput.setAttribute("placeholder", window.parent.DiscoBoard.backendMethods.getAppDetails(e.target.getAttribute("packagename"), true).label)
            ...
            if (appdetail.packageName.startsWith("disco.internal")) {
...
function updateAppDetailsRender(packageName) {
    const appPreference = window.parent.DiscoBoard.backendMethods.getAppPreferences(packageName)
    const rawAppDetails = window.parent.DiscoBoard.backendMethods.getAppDetails(packageName, true)
    ...
    const listTitle = document.querySelector(`div.disco-list-view-item[packagename='${packageName}'] > p.disco-list-view-item-title`)
    if (listTitle) listTitle.innerText = label
    window.parent.DiscoBoard.backendMethods.reloadApps()
    const homeTile = window.parent.document.querySelector(`div.disco-home-tile[packagename='${packageName}']`)
    if (homeTile) {
        homeTile.setAttribute("title", label)
        homeTile.querySelector("p.disco-home-tile-title").innerText = label
    }

    if (window.parent.DiscoBoard) {
        refreshAppList()
    }
}
...
function setupIconDropdown(appdetail, tilePrefs) {
    ...
    try {
        if (window.parent.Disco && window.parent.Disco.getIconPacks) {
            const iconPacks = JSON.parse(window.parent.Disco.getIconPacks());
...
function applyTilePreferencesToApp(packageName, prefs) {
    try {
        ...
        const homeTile = window.parent.document.querySelector(`div.disco-home-tile[packagename='${packageName}']`);
        const appListItem = window.parent.document.querySelector(`div.disco-app-tile[packagename='${packageName}']`);
        ...
        [homeTile, appListItem].forEach(element => {
            if (element) {
                ...
                // Use the main DiscoBoard function to apply preferences
                if (window.parent.DiscoBoard && window.parent.DiscoBoard.backendMethods.applyTilePreferences) {
                    window.parent.DiscoBoard.backendMethods.applyTilePreferences(element, packageName);
                }
            }
        });
...
function applyTilePreferencesToElement(element, effectivePrefs, packageName) {
    ...
    const titleElement = element.querySelector(".disco-home-tile-title, .disco-app-tile-title");
    if (titleElement) {
...
function getAppTilePreferences(packageName) {
    try {
        if (window.parent.Disco && window.parent.Disco.getAppTilePreferences) {
            const prefsStr = window.parent.Disco.getAppTilePreferences(packageName);
...
function getGlobalTilePreferences() {
    // Get global preferences from Disco Tweaks
...
function setAppTilePreferences(packageName, data) {
    try {
        if (window.parent.Disco && window.parent.Disco.setAppTilePreferences) {
            window.parent.Disco.setAppTilePreferences(packageName, JSON.stringify(data));
            //console.log("Saved app tile preferences via Disco API:", packageName, data);
        }
    } catch (error) {
        console.log("Error setting app tile preferences via Disco API:", error);
    }
