import { discoThemes } from "../../../scripts/DiscoProperties";
import imageStore from "../../../scripts/imageStore";
import i18n from "../../../scripts/localeManager";
import iro from '@jaames/iro';

const urlParams = new URLSearchParams(window.location.search);
const settingsListTag = document.querySelector("#home-tab > div:nth-child(1) > div > div:nth-child(1) > p.disco-list-view-item-description")
...
document.getElementById("theme-chooser").addEventListener('selected', (e) => {
    appViewEvents.setTheme(e.detail.index == 0 ? discoThemes.light : e.detail.index == 1 ? discoThemes.dark : discoThemes.auto)
});
...
document.querySelector("div.color-picker").addEventListener("flowClick", (e) => {
    ...
    if (Disco.getSystemAccentColor("supported") == "true") {
        systemColorTile.style.visibility = "visible"
        systemColorTile.style.background = Disco.getSystemAccentColor()
        systemColorTile.innerHTML = '<p class="system-accent-color"></p>'
        systemColorTile.querySelector("p.system-accent-color").innerText = Disco.getSystemAccentColor("provider")
    } else {
...
document.querySelectorAll("div.accent-color-catalogue-item").forEach(e => e.addEventListener("flowClick", (e) => {
    if (e.target.id == "custom-color-item") {
        ...
        window.parent.DiscoBoard.backendMethods.navigation.push("settings-inner-page:accent-color-picker:custom-color", () => { }, () => {
        ...
        window.parent.DiscoBoard.backendMethods.navigation.invalidate("settings-inner-page:accent-color-picker")
        clearTimeout(window.accentColorPickerTimeout)
        accentColorPicker.classList.remove("shown")
        accentColorPicker.classList.add("hidden")
        window.accentColorPickerTimeout = setTimeout(() => {
        ...
        appViewEvents.setAccentColor(discoColors[e.target.style.background.slice(18).slice(0, -1)])
...
document.getElementById("remove-wallpaper").addEventListener("flowClick", (e) => {
    ...
    document.getElementById("remove-wallpaper").style.visibility = "hidden"
    window.parent.DiscoBoard.backendMethods.wallpaper.remove()
})
...
setTimeout(() => {
    if (!localStorage['accentColor']) {
        ...
        const colorName = Object.keys(discoColors).find(key => discoColors[key] == localStorage['accentColor']) || "custom_color";
...
applyColorButton.addEventListener("flowClick", () => {
    ...
    window.parent.DiscoBoard.backendMethods.navigation.invalidate("settings-inner-page:accent-color-picker:custom-color")
    window.parent.DiscoBoard.backendMethods.navigation.invalidate("settings-inner-page:accent-color-picker")
    document.querySelector("div.custom-color-selector").style.setProperty("display", "none")
    document.querySelector("div.accent-color-catalogue").style.removeProperty("display")
    clearTimeout(window.accentColorPickerTimeout)
    ...
    if (Object.values(discoColors).includes(newColor)) {
        colorName = Object.keys(discoColors).find(key => discoColors[key] == newColor)
    }
