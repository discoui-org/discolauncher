...
// Update discoMock.js
const discoMockPath = path.join(__dirname, '..', 'src', 'scripts', 'discoMock.js');
let discoMockContent = fs.readFileSync(discoMockPath, 'utf8');

// Replace version name between comment markers
const versionNameRegex = /(\/\/ VERSIONNAME START[\s\S]*?return ")[^"]*(";[\s\S]*?\/\/ VERSIONNAME END)/;
discoMockContent = discoMockContent.replace(versionNameRegex, `$1${versionName}$2`);

fs.writeFileSync(discoMockPath, discoMockContent);
console.log('Updated discoMock.js version');
